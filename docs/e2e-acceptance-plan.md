...
| A1.7 | Build identity in the footer and on the org page | **OK** (23 Aug, 0.91.0). Both carry it. The footer reads `Sextant <version> - NixOS fleet control plane`, and `/org` opens with a "This deployment" block whose first row is Version, with the sentence "The release this console was built as. Quote it in a support request". The metric that made this a FAIL is fixed too: the running production pod reports `sextant_build_info{version="0.91.0"}` where it used to say `dev`, which was the defect issue #42 named. One caveat for whoever reads a local console: `just demo` builds without ldflags, so the footer there says `Sextant dev` and that is correct rather than a regression. The production pages themselves were not read logged in - the login page uses a layout without the footer - so what is measured here is the template on 0.91.0, the demo rendering it, and the version the production binary reports about itself |
