...
| A4.3 | Force comin-config-guard | **NOT PROVED, and the attempt was informative** (11 Aug). Changing the station's poll interval was meant to leave comin running a stale config, since its unit carries `X-RestartIfChanged=false`. It did not: comin restarted itself, `restart_comin=True` in its own store, at the same second the deployment ended. So comin covers that case and the guard covers a disjoint one - `restart_comin` fires when comin *performs* a deployment that changes its config, while the failure the guard exists for (test15 following `main` for weeks) is the case where comin performs no deployment at all. Proving this row needs comin started against a config path no deployment can reach, which needs root on the device |
