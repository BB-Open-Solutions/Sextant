...
| A1.7 | Build identity in the footer and on the org page | **OK** (23 Aug, 0.91.0). Both carry it. The footer reads `Sextant <version> - NixOS fleet control plane`, and `/org` opens with a "This deployment" block whose first row is Version, with the sentence "The release this console was built as. Quote it in a support request". The metric that made this a FAIL is fixed too: the running production pod reports `sextant_build_info{version="0.91.0"}` where it used to say `dev`, which was the defect issue #42 named. Read from a released build rather than inferred: the container path (`sextant:0.91.0` on podman) renders `Sextant 0.91.0 - NixOS fleet control plane` in its footer, so the version reaches a rendered page and not only a metric. One caveat for whoever reads a local console: `just demo` builds without ldflags, so the footer there says `Sextant dev`, which is correct rather than a regression. What was not done is reading console.bb-open.com under a session - the login page uses a layout without the footer, so an unauthenticated request cannot see it |
