...
func sampleRows() []deviceRow {
	return []deviceRow{
		{Tag: "lt-ada", Class: "laptop", Hardware: "t495s", AssignedUser: "ada", Groups: []string{"pilot"}, HasStatus: true, Online: true},
		{Tag: "srv-1", Class: "server", Hardware: "msi", Groups: []string{"zaanstad"}, HasStatus: true, Online: false},
		{Tag: "lt-new", Class: "laptop", Hardware: "hp-g4", Groups: []string{"pilot"}},
	}
}
...
func TestFilterDeviceRows(t *testing.T) {
	cases := []struct {
		...
		{"combined", "lt", "laptop", "pilot", "online", []string{"lt-ada"}},
		{"no match", "nope", "", "", "", []string{}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := rowTags(filterDeviceRows(sampleRows(), c.q, c.class, c.group, c.status))
...
func TestSortDeviceRows(t *testing.T) {
	mk := func() []deviceRow {
		return []deviceRow{
			{Tag: "b", Hardware: "z", HasStatus: true, Online: false},
			{Tag: "a", Hardware: "y", HasStatus: true, Online: true},
			{Tag: "c", Hardware: "x", HasStatus: false},
		}
	}
	cases := []struct {
		key, dir string
