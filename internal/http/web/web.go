...
type Services struct {
	...
	// Erasure removes one person's data on request (GDPR art. 17); nil
	// leaves the page saying why rather than hiding it, because "we cannot
	// do that here" is an answer a DPO needs.
...
func (s *Server) Routes(mux *http.ServeMux) {
	...
	post("/org/erasure/preview", s.postErasurePreview)
	post("/org/erasure/confirm", s.postErasureConfirm)
