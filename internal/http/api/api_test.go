...
func newTestAPI(t *testing.T, write bool) *httptest.Server {
	...
	New(svc, testToken, write, slog.New(slog.NewTextHandler(io.Discard, nil))).Routes(mux)
...
func TestGateRejectionIs422(t *testing.T) {
	...
	mux := http.NewServeMux()
	New(svc, testToken, true, slog.New(slog.NewTextHandler(io.Discard, nil))).Routes(mux)
	srv := httptest.NewServer(mux)
	defer srv.Close()
...
func TestDisabledAPIWithoutToken(t *testing.T) {
	...
	mux := http.NewServeMux()
	New(svc, "", true, slog.New(slog.NewTextHandler(io.Discard, nil))).Routes(mux)
	srv := httptest.NewServer(mux)
	defer srv.Close()
